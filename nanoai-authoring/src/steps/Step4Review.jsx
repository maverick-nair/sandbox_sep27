  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-xl font-semibold"><InlineText value={sc.title} onCommit={(v) => setScenario({ title: v }, 'scenario.title', { after: v })} ariaLabel="scenario title" /></h1>
        ...
        {sc.pendingConfirmation && (
          <Panel tone="warn" title="The situation or media changed, so the analysis re-ran" subtitle="Confirm the changed scoring questions or revert to the previous set." padding="p-4">
            <ul className="space-y-1 text-sm">{sc.pendingConfirmation.changed.map((c) => <li key={c.id} className="flex gap-2"><Badge tone={c.status === 'unchanged' ? 'neutral' : c.status === 'removed' ? 'block' : 'warn'}>{c.status}</Badge><span>{c.text}{c.status === 'changed' && c.previous && <span className="faint block text-xs">was: {c.previous}</span>}</span></li>)}</ul>
          ...
          <div className="space-y-4">
            <Panel title="What the participant sees" subtitle="Edit any text inline. Editing the situation re-runs the contextual analysis." padding="p-5">
              <div className="space-y-3">
                <div><div className="faint mb-1 text-[11px] uppercase tracking-wider">Context header <span className="normal-case">({wordCount(sc.contextHeader)} of 30 words)</span></div><InlineText value={sc.contextHeader} onCommit={(v) => editSituation('contextHeader', v)} multiline className="text-sm font-medium" ariaLabel="context header" /></div>
                <div><div className="faint mb-1 flex items-center justify-between text-[11px] uppercase tracking-wider"><span>Situation <span className="normal-case">({wordCount(sc.situation)} words, grade {grade} reading level)</span></span>{!readOnly && <button className="normal-case text-[var(--brand)]" onClick={() => setRegen({ scope: 'sentence', sentenceIndex: 0, label: 'one sentence' })}>Regenerate one sentence</button>}</div><InlineText value={sc.situation} onCommit={(v) => editSituation('situation', v)} multiline className="text-[15px] leading-relaxed" ariaLabel="situation" /></div>
                ...
                <div><div className="faint mb-1 text-[11px] uppercase tracking-wider">Prompt</div><InlineText value={sc.prompt} onCommit={(v) => setScenario({ prompt: v, approved: false }, 'scenario.prompt', { after: v })} className="text-sm font-medium" ariaLabel="prompt" /></div>
            ...
            <Panel title="Contextual analysis" subtitle="What the AI read in the situation and the media. Scoring questions are derived from this." padding="p-5">
              <div className="grid gap-4 text-sm sm:grid-cols-2">
                ...
                {sc.responseType !== 'MCQ' && <div className="sm:col-span-2"><div className="faint mb-1 text-[11px] uppercase tracking-wider">Model answer <span className="normal-case">({wordCount(sc.analysis?.modelAnswer || '')} words · sizes the response limit)</span></div><InlineText value={sc.analysis?.modelAnswer} onCommit={(v) => setScenario((s) => ({ ...s, approved: false, analysis: { ...s.analysis, modelAnswer: v } }), 'analysis.model_answer_edited')} multiline className="text-sm" ariaLabel="model answer" /></div>}
            ...
            {sc.responseType !== 'MCQ' ? (
                  ...
                  {sc.scoringQuestions.map((q, qi) => { const ind = indicatorById(q.indicatorId); return (
                    <li key={q.id} className="p-4">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1"><div className="flex items-baseline gap-2"><span className="faint text-xs">{qi + 1}.</span><InlineText value={q.text} onCommit={(v) => editQuestion(q.id, { text: v }, 'scoring_question.text_edited')} className="text-sm font-medium" ariaLabel={`scoring question ${qi + 1}`} /></div>
              ...
              <Panel title={`Questions and options (${sc.mcq.length} of up to 3)`} subtitle="Up to 4 options, all plausible, each written to one proficiency level with a keyed value of 1 to 5 and a rationale that reads as a coaching note. Editing an option re-keys it." right={!readOnly && <Button size="sm" variant="ghost" onClick={addMcqQuestion}>Add question</Button>} padding="p-0">
                <ol className="divide-y divide-[var(--line)]">
                  {sc.mcq.map((q, qi) => (
                    <li key={q.id} className="p-4">
                      <div className="flex items-start justify-between gap-2"><div className="flex items-baseline gap-2"><span className="faint text-xs">Q{qi + 1}.</span><InlineText value={q.text} onCommit={(v) => setScenario((s) => ({ ...s, approved: false, mcq: s.mcq.map((x) => (x.id === q.id ? { ...x, text: v } : x)) }), 'mcq.question_edited', { after: v })} className="text-sm font-medium" ariaLabel={`MCQ question ${qi + 1}`} /></div>{!readOnly && <div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => setRegen({ scope: 'mcqQuestion', targetId: q.id, label: `question ${qi + 1}` })}>Regenerate</Button><Button size="sm" variant="ghost" onClick={() => removeMcqQuestion(q.id)}>Remove</Button></div>}</div>
                      <ol className="mt-2 space-y-2">
                        {q.options.map((o, oi) => (
                          <li key={o.id} className="rounded-lg border border-[var(--line)] p-2.5">
                            <div className="flex items-start gap-2"><span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-100 text-[11px] font-semibold">{String.fromCharCode(65 + oi)}</span><div className="flex-1"><InlineText value={o.text} onCommit={(v) => editOption(q.id, o.id, { text: v }, true)} multiline className="text-sm" ariaLabel={`option ${String.fromCharCode(65 + oi)}`} /><div className="mt-1.5 flex flex-wrap items-center gap-2 text-xs"><label className="flex items-center gap-1"><span className="faint">Value</span><Select disabled={readOnly} value={o.key} onChange={(e) => setKey(q.id, o.id, e.target.value)} className="w-16 py-0.5 text-xs" aria-label="Keyed value">{[1, 2, 3, 4, 5].map((k) => <option key={k} value={k}>{k}</option>)}</Select></label><Badge tone={o.key >= 4 ? 'ok' : o.key <= 2 ? 'block' : 'warn'}>{LEVEL_LABELS[o.level] || o.level}</Badge>{o.rekeyed && <Badge tone="brand">Re-keyed {o.rekeyed.before} to {o.rekeyed.after}</Badge>}{!readOnly && <button className="text-[var(--brand)]" onClick={() => setRegen({ scope: 'option', targetId: q.id, optionId: o.id, label: `option ${String.fromCharCode(65 + oi)}` })}>Regenerate option</button>}{!readOnly && q.options.length > 2 && <button className="muted" onClick={() => removeOption(q.id, o.id)}>Remove</button>}</div><div className="muted mt-1 text-xs"><span className="faint">Why: </span><InlineText value={o.rationale} onCommit={(v) => editOption(q.id, o.id, { rationale: v }, false)} multiline ariaLabel="rationale" placeholder="Why does this option earn its value? Name the behavior." /></div></div></div>
                          </li>
                        ))}
                      </ol>
      ...
      <Modal open={Boolean(regen)} title={`Regenerate ${regen?.label || ''}`} onClose={() => setRegen(null)} footer={<><Button variant="secondary" onClick={() => setRegen(null)}>Cancel</Button><Button onClick={runRegen} busy={Boolean(busy)}>Regenerate</Button></>}>
        ...
        <p className="faint mt-2 text-xs">{llmAvailable() ? 'The AI rewrites only what you asked for and re-runs the analysis where the facts change. Undo is available.' : 'Scripted mode: instructions of the form "make this about X, not Y" or "replace X with Y" are applied literally; whole scenario regeneration rotates the library. Connect an API key for free form regeneration.'}</p>
      </Modal>
