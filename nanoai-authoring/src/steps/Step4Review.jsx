          ...
          <aside aria-label="Response settings and Skill" className="space-y-4">
            <Panel as="h3" title="Response and limits" subtitle="The AI recommended these from the complexity and the model answer. Adjust within range." padding="p-4">
              <div className="space-y-4 text-sm">
                <div><div className="faint mb-1 text-[11px] uppercase tracking-wider">Response type</div><div className="flex gap-1">{RESPONSE_TYPES.map((t) => <button key={t} disabled={readOnly || Boolean(busy)} onClick={() => t !== sc.responseType && switchType(t)} aria-pressed={sc.responseType === t} className={`flex-1 rounded-lg border px-2 py-1.5 text-xs font-medium ${sc.responseType === t ? 'border-[var(--brand)] bg-[var(--brand-soft)] text-[var(--brand)]' : 'border-[var(--line)] hover:bg-[var(--card-3)]'}`}>{t}</button>)}</div><p className="faint mt-1 text-xs">{sc.responseType === 'Audio' ? 'Recorded answer, transcribed and scored on content.' : sc.responseType === 'Text' ? 'Typed answer with a live character count.' : 'Single select per question, no cap.'} Switching rebuilds the scoring instrument and recomputes observations and time.</p></div>
                ...
                {sc.responseType !== 'MCQ' && <div className="rounded-lg bg-[var(--card-2)] p-2 text-xs"><span className="font-medium">AI scoring calibration:</span> {sc.calibration === 'complete' ? 'complete' : 'pending. Before AI scoring activates for this scenario, 30 responses are scored by two calibrators and AI to human agreement must reach 0.75 per question.'}</div>}
              </div>
            </Panel>
