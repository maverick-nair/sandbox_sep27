    ...
    <div className="grid gap-5 lg:grid-cols-[1fr_380px]">
      ...
      <aside className="space-y-4">
        ...
        <Panel title="Versions" subtitle="Each version pins ontology, model and prompt versions. Historical scores are never recomputed silently." padding="p-0">
          {asm.versions.length === 0 ? <p className="muted p-4 text-sm">Not yet published.</p> : <ul className="divide-y divide-[var(--line)]">{[...asm.versions].reverse().map((v) => <li key={v.version} className="p-3 text-sm"><div className="flex items-center justify-between"><span className="font-medium">Version {v.version}</span><Badge tone={v.reviewStatus === 'published' ? 'ok' : 'warn'}>{v.reviewStatus === 'published' ? 'Live' : 'KNOLSKAPE review'}</Badge></div><div className="muted text-xs">{new Date(v.publishedAt).toLocaleString()} · {v.scenarios.length} scenarios · {v.ontologyVersion} · {v.modelVersion}</div><div className="mt-1.5 flex gap-2"><Button size="sm" variant="ghost" onClick={() => exportPackage(v)}>Export package</Button>{v.reviewStatus !== 'published' && !readOnly && <Button size="sm" variant="ghost" onClick={() => update((a) => ({ ...a, versions: a.versions.map((x) => (x.version === v.version ? { ...x, reviewStatus: 'published' } : x)) }), { action: 'version.review_completed', after: v.version })}>Mark review complete (demo)</Button>}</div></li>)}</ul>}
        </Panel>
