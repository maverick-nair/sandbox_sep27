    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-center">
        <Frame className={device === 'mobile' ? 'phone-frame' : 'desktop-frame'}>
          <div className="participant flex h-full flex-col overflow-auto bg-white" style={{ maxHeight: device === 'mobile' ? 760 : undefined }}>
...
function ScenarioScreen({ sc, index, count, remaining, onSubmit, onPause }) {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="sticky top-0 z-10 border-b border-[var(--line)] bg-white/95 px-5 py-2 text-xs backdrop-blur"><div className="flex items-center justify-between"><span className="font-medium">Scenario {index + 1} of {count}</span><span className="muted">About {remaining} min remaining</span></div><div className="mt-1 flex items-center gap-2"><Progress value={index} max={count} /><span className={`whitespace-nowrap font-mono ${left < 0 ? 'text-[var(--warn)]' : 'muted'}`} aria-live="polite">{left >= 0 ? `${Math.floor(left / 60)}:${String(left % 60).padStart(2, '0')} suggested` : 'Over the suggested time'}</span></div></div>
      <div className="flex-1 space-y-4 p-5">
        <div className="rounded-lg bg-slate-50 p-3 text-sm"><span className="faint mr-1 text-[11px] uppercase tracking-wider">Your situation</span>{sc.contextHeader}</div>
...
function Recorder({ capSeconds, onSubmit, practice }) {
  ...
  return (
    <div className="card p-4">
      {state === 'idle' && <div className="flex flex-col items-center gap-2"><button onClick={start} className="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--block)] text-white shadow focus:outline-none focus-visible:ring-4 focus-visible:ring-[var(--block)]/30" aria-label="Start recording"><span className="h-6 w-6 rounded-full bg-white" /></button><span className="text-sm">Tap to record. Up to {formatSeconds(capSeconds)}. Recording never starts without a tap.</span>{err && <p className="text-xs text-[var(--block)]">{err}</p>}</div>}
      {state === 'recording' && <div className="flex flex-col items-center gap-2"><button onClick={stop} className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-800 text-white" aria-label="Stop recording"><span className="h-5 w-5 rounded-sm bg-white" /></button><div className="h-2 w-40 rounded bg-slate-200"><div className="h-2 rounded bg-[var(--ok)] transition-all" style={{ width: `${Math.round(level * 100)}%` }} /></div><span className={`font-mono text-sm ${remaining <= 15 ? 'text-[var(--warn)]' : ''}`} aria-live="polite">{formatSeconds(remaining)} left{remaining <= 15 ? '. Recording will stop at the limit.' : ''}</span>{transcript && <p className="muted text-xs italic">{transcript}</p>}</div>}
      {state === 'review' && (
