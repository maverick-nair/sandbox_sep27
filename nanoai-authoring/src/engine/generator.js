...
async function llmScenario(row, asm, index, avoid) {
  ...
  return {
    ...
    contextHeader: j.contextHeader, situation: j.situation, prompt: j.prompt, media, analysis: null, scoringQuestions: [], mcq: [], approved: false,
...
function placeholderScenario(row, index, reason) {
  return { id: uid('sc'), blueprintRowId: row.id, skillId: row.skillId, tag: row.tag, difficulty: row.difficulty, responseType: row.responseType, title: `Scenario ${index + 1} (not generated)`, contextHeader: '', situation: '', prompt: '', media: null, analysis: null, scoringQuestions: [], mcq: [], approved: false, calibration: row.responseType === 'MCQ' ? 'not_applicable' : 'pending', allowedTerms: [], version: 1, generatedBy: 'none', promptVersion: PROMPT_VERSION, modelVersion: null, generationError: reason, source: { kind: 'error', text: reason }, pendingConfirmation: null };
}
