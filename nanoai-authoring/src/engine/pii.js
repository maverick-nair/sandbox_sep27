...
const NAME_EDGE = new Set(['Customer', 'Employee', 'Client', 'Manager', 'Contact', 'Later', 'Then', 'Dear', 'Regards', 'Thanks', 'Hello', 'Hi', 'The', 'Our', 'Your', 'When', 'After', 'Before', 'Meanwhile', 'Today', 'Yesterday', 'Colleague', 'Agent', 'Caller', 'Patient', 'From', 'With', 'Named', 'Analyst', 'Director', 'Supervisor', 'Team', 'Lead', 'Officer', 'Nurse', 'Driver', 'Rep', 'Signed', 'Sincerely', 'Cc', 'To', 'Re', 'Subject', 'Note', 'Update', 'Summary', 'Store', 'Shop', 'Group', 'Retail', 'Connect', 'Systems', 'Services', 'Solutions', 'Bank', 'Hospital', 'University', 'Limited', 'Ltd', 'Inc', 'Plc', 'Global', 'Digital', 'Cloud', 'Platform', 'Office', 'Regional', 'Head', 'Sales', 'Finance', 'Marketing', 'Operations', 'Support', 'Product', 'Project', 'Programme', 'Program', 'Assessment', 'Skills', 'Works', 'Assist', 'Nano', 'Genie', 'Kreator', 'Portal', 'Hub', 'Centre', 'Center', 'Quarter', 'Region', 'North', 'South', 'East', 'West', 'Board', 'Committee']);
...
export function detectPII(text = '', { allowNames = [] } = {}) {
  ...
  for (const m of text.matchAll(FULL_NAME)) {
    ...
    if (NAME_STOP.has(value) || words.some((w) => NAME_EDGE.has(w)) || allow.has(value.toLowerCase()) || allow.has(words[0].toLowerCase())) continue;
    ...
    if (cueWord || FIRST_NAMES.has(words[0].toLowerCase()) || /(?:employee|customer|client|manager|contact|named|colleague|agent|rep|caller|patient|ms|mr|dear|regards|thanks|signed)[\s,:-]*$/i.test(before) || /^\s*(?:\(|<|wrote|said|called|emailed|reported|complained|asked|told)/.test(after)) push({ kind: 'PII', type: 'NAME', start, end: start + value.length, value, placeholder: '[Person]' });
  }
