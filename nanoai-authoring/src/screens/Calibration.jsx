  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
      <aside aria-label="Open response scenarios" className="space-y-2">
        ...
        <ol className="space-y-1">{open.map((s) => { const c = asm.calibration?.[s.id]; const st = calibrationStatus(s, c); return <li key={s.id}><button onClick={() => setSelId(s.id)} aria-current={s.id === sc.id} className={`w-full rounded-lg border p-2 text-left text-xs ${s.id === sc.id ? 'border-[var(--brand)] bg-[var(--brand-soft)]' : 'border-[var(--line)] bg-[var(--card)] hover:bg-[var(--card-3)]'}`}><div className="flex items-center justify-between gap-1"><span className="font-medium">{s.title}</span><Badge tone={{ complete: 'ok', in_progress: 'warn', paused: 'block', pending: 'neutral' }[st]}>{st.replace('_', ' ')}</Badge></div><div className="faint mt-0.5">{getSkill(s.skillId)?.name} · {s.responseType} · {c?.responses.length || 0}/{RULES.calibration.responses} responses</div></button></li>; })}</ol>
