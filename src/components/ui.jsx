...
export function Gauge({ label, value, unit = '', target, targetLabel, good, max = 100, format }) {
  ...
  return (
    <div className="rounded border border-zinc-800 bg-zinc-950/60 p-3">
      ...
      {target !== undefined && <div className="mt-1 text-[11px] text-zinc-400">Target {targetLabel || `${target}${unit}`}</div>}
    </div>
  );
}
    ...
    <dl className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      {items.map((it) => (
        <div key={it.label} className="rounded border border-zinc-800 bg-zinc-950/60 px-3 py-2">
          <dt className="text-[10px] uppercase tracking-wider text-zinc-400">{it.label}</dt>
