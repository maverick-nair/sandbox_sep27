...
export function scoreLaunch({ dossier, prd, answers, judgeResult, meters, weakest }) {
  ...
  return {
    ...
    detail: { dossier, answers, judgeResult, outcome: outcome.id, weakest, filled: filled.length },
  };
}
...
export default function Level8Launch() {
  ...
  const draft = state.levelDraft[8] || {};
    ...
    <div>
      ...
      {stage === 'dossier' && !finalResult && (
        <DnDProvider onDrop={onDrop}>
          <div className="grid gap-4 lg:grid-cols-[300px_1fr]">
            <DropZone id="artifacts" label="Completed artifacts" className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-3">
              ...
              <div className="space-y-1.5">
                ...
                {DOSSIER_SLOTS.filter((s) => !state.prd[s.prdKey]).map((s) => <div key={s.id} className="rounded border border-dashed border-zinc-800 p-2 text-xs text-zinc-600">{s.label}: never produced ({s.source})</div>)}
                {available.length === 0 && DOSSIER_SLOTS.every((s) => !state.prd[s.prdKey] || dossier[s.id]) && <p className="text-xs text-zinc-500">All available artifacts placed.</p>}
              </div>
            </DropZone>
              ...
              {DOSSIER_SLOTS.map((s) => {
                ...
                return (
                  <DropZone key={s.id} id={s.id} label={`Dossier slot: ${s.label}`} className={`min-h-[96px] rounded-lg border p-3 ${art ? (wrong ? 'border-sky-500/50 bg-sky-500/5' : 'border-amber-500/40 bg-amber-500/5') : 'border-dashed border-zinc-700'}`}>
                    ...
                    {art ? <ArtifactCard slot={art} text={state.prd[art.prdKey]} compact /> : <p className="mt-1 text-xs text-zinc-600">Empty. The board will flag it.</p>}
          ...
          <p className="mt-2 text-xs text-zinc-500">Keyboard: Enter selects an artifact, Tab to a slot, Enter places it.</p>
          <div className="mt-4 flex items-center justify-end gap-3">
            <span className="text-xs text-zinc-500">{Object.keys(dossier).length}/{DOSSIER_SLOTS.length} slots filled.</span>
          ...
          <div className="mt-4 space-y-4">
            {questions.slice(0, qIndex + 1).map((q, i) => (
              <div key={i} className="rounded border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-wider text-zinc-500">Question {i + 1} of 3</div>
                ...
                {i < qIndex ? (
                  ...
                  <>
                    ...
                    <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-zinc-500">
                      <span>{wordCount(answers[i])} words (40 minimum). {llmAvailable() ? 'Elena responds live.' : 'No API key: scripted acknowledgements.'}</span>
      ...
      {finalResult && outcome && (
        <>
          ...
          <LevelResult level={8} result={finalResult} onContinue={() => window.dispatchEvent(new CustomEvent('lw:next'))} />
        </>
      )}
    </div>
  );
...
function ArtifactCard({ slot, text, compact }) {
  return (
    <Draggable id={slot.id} label={`${slot.label} artifact`} className={`rounded border border-zinc-700 bg-zinc-950 ${compact ? 'mt-2 p-2 text-[11px]' : 'p-2 text-xs'}`}>
      <div className="font-medium text-zinc-100">{slot.label} <span className="text-zinc-500">({slot.source})</span></div>
