...
export function scoreEvalLab({ classifications, golden, threshold, decision, judgeResult, seconds }) {
  ...
  return {
    ...
    detail: { accuracy: Math.round(accuracy), goldenScore: Math.round(goldenScore), thresholdScore, decisionScore: Math.round(decisionScore), speedBonus, judgeResult, heldOrHighThreshold },
  };
}

export default function Level3EvalLab() {
  const { state, log, complete, saveDraft } = useGame();
  const draft = state.levelDraft[3] || {};
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {[1, 2, 3, 4].map((n) => <button key={n} onClick={() => setTask(n)} className={`rounded border px-3 py-1 text-xs ${task === n ? 'border-amber-400 text-amber-200' : 'border-zinc-700 text-zinc-400'}`}>Task {n}: {['Classify traces', 'Build golden set', 'Set threshold', 'Ship or hold'][n - 1]}</button>)}
      </div>
        ...
        <DnDProvider onDrop={onDrop}>
          <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
            <DropZone id="tray" label="Unclassified traces" className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-3">
              <div className="mb-2 flex items-center justify-between"><span className="text-xs uppercase tracking-wider text-zinc-400">Trace inspector</span><span className="font-mono text-xs text-zinc-500">{unclassified.length} unclassified</span></div>
              <div className="grid gap-2 md:grid-cols-2">
                ...
                {unclassified.length === 0 && <p className="text-sm text-zinc-500">All traces classified. Move to Task 2.</p>}
              </div>
            </DropZone>
              ...
              {ERROR_BUCKETS.map((b) => {
                ...
                return (
                  <DropZone key={b.id} id={b.id} label={b.label} className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-2.5">
                    <div className="flex items-center justify-between"><span className="text-xs font-medium text-zinc-200">{b.label}</span><span className="font-mono text-xs text-zinc-500">{items.length}</span></div>
          ...
          <p className="mt-2 text-xs text-zinc-500">Keyboard: Enter selects a trace, Tab to a bucket, Enter places it. Speed bonus applies above 70 percent accuracy.</p>
        </DnDProvider>
      )}
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {TRACES.map((t) => (
              <button key={t.id} disabled={Boolean(finalResult)} onClick={() => toggleStar(t.id)} aria-pressed={golden.includes(t.id)} className={`rounded border p-2.5 text-left text-xs ${golden.includes(t.id) ? 'border-amber-400 bg-amber-500/10' : 'border-zinc-800 bg-zinc-950'} focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400`}>
                <div className="flex items-center justify-between"><span className="font-mono text-zinc-400">{t.id}</span><span className="text-zinc-500">Your label: {classifications[t.id] ? ERROR_BUCKETS.find((b) => b.id === classifications[t.id]).label : 'none'}</span></div>
      ...
      {task === 3 && (
        <Panel title="Ship threshold" subtitle="Higher threshold means a later ship date and more Team Hours. The trade-off updates live.">
          ...
          <p className="mt-2 text-xs text-zinc-500">Formula: hours = 6 per point above {CURRENT_PASS_RATE}; weeks = 0.35 per point; incident risk = 100 minus 0.95 times threshold.</p>
        </Panel>
      )}
      ...
      {task === 4 && (
        <Panel title="Ship or hold" subtitle="A 100 word justification is required. The judge scores evidence use, not confidence.">
          ...
          <div className="mt-1 flex items-center justify-between text-xs text-zinc-500"><span>{wordCount(justification)} words (100 required)</span><span>{llmAvailable() ? 'LLM judge active' : 'No API key: neutral judge score will be applied'}</span></div>
          {!finalResult && (
          ...
          {!finalResult && (Object.keys(classifications).length < TRACES.length || golden.length < GOLDEN_SET_SIZE) && <p className="mt-2 text-xs text-sky-300">Complete Task 1 (all 20 classified) and Task 2 ({GOLDEN_SET_SIZE} starred) before submitting.</p>}
        </Panel>
      )}

      {finalResult && <LevelResult level={3} result={finalResult} onContinue={() => window.dispatchEvent(new CustomEvent('lw:next'))} />}
    </div>
  );
}
...
function TraceCard({ trace, expanded, onToggle, disabled }) {
  return (
    <Draggable id={trace.id} label={`Trace ${trace.id}: ${trace.question}`} disabled={disabled} className="rounded border border-zinc-700 bg-zinc-950 p-2.5 text-xs">
      ...
      <div className="mt-1 text-zinc-100"><span className="text-zinc-500">Q: </span>{trace.question}</div>
      <div className="mt-1 text-zinc-300"><span className="text-zinc-500">A: </span>{trace.answer}</div>
      <div className="mt-1 text-zinc-500">Citation: {trace.citation || 'none'}</div>
      {expanded && <div className="mt-2 rounded border border-zinc-800 bg-zinc-900 p-2 text-zinc-300"><span className="text-zinc-500">Retrieved context: </span>{trace.context}</div>}
    </Draggable>
  );
}
