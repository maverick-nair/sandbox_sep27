...
_Original verdict, before the fixes below:_
...
## Fix status (re-test on this branch)

All 32 issues were addressed. 30 are fixed; QA-23 and QA-31 are fixed as far as a browser prototype allows, with the remaining production work listed. Re-test: 45 unit tests pass (13 new regression tests in `tests/qa-fixes.test.mjs`, one per finding group), and a 59-check Playwright run (`qa-verify`) passes in Chromium at desktop and 360 px touch sizes, alongside the original journey scripts.

| ID | Status | What changed | How it was verified |
|---|---|---|---|
| QA-01 | Fixed | Real save state in the Studio header (Saving, Saved in this browser, Not saved). A failed write shows a persistent banner with Try again and Download a backup. Only the newest 5 published versions keep a full copy; on a quota error the store compacts to 2 and retries. | Browser: quota forced, banner and backup shown. Unit: version compaction. |
| QA-02 | Fixed | The product or service name is asked when it cannot be inferred, with a suggestion drafted from the organization (for example Solaris Plus). Blank context values are never written into the draft; an empty field renders as a visible [Label] placeholder and is a blocking health check error. | Browser: solar brief letter has no gaps. Unit: placeholder and health check. |
| QA-03 | Fixed | Places are weighted by phrasing: "in", "based in", "team in" count as the team; "clients in", "selling to" count as customers. US states and state codes are recognised (Atlanta, Georgia; Austin, TX). Cities need a location phrase, so "CEO David" and "Florence, our L&D head" are no longer places. Two team locations, or Georgia on its own, become a question with one-click options, labelled Inferred until answered. | Unit and browser. |
| QA-04 | Fixed | The creation flow autosaves every change. Simulations shows Resume where you left off (with Discard), and starting a new one while a draft exists asks first. The flow shows "Your progress is saved"; leaving only warns while Genie is reading or saving has failed. | Browser: reload on step 3 resumes with the edit intact. |
| QA-05 | Fixed | App-level error boundary with Back to all simulations, Reload and Download a backup. Imports are checked against the full definition shape (schemaProblems) and migrated before use; stored records that fail the check are skipped instead of crashing the list. | Browser: partial import rejected with reasons; corrupted stored record does not blank the app. |
| QA-06 | Fixed | A missing organization is left empty with the sample only as a placeholder and a Use Meridian Bank button. A question counts as answered only when it has a value. Picking an industry no longer fills open questions with sample names. | Browser: empty after type and clear still blocks Continue. |
| QA-07 | Fixed | The reader extracts weeks, target, stage names, team size, team names, requested events and deal value (rules and the Genie schema). Used items are applied (8 weeks, target 60, the five stages, Ravi and Sita with matching pronouns, an RBI audit event in week 3, INR 25,000); items the template cannot take (team of 8) are listed as Not used yet with the reason, on the Context and Review steps. | Unit and browser. |
| QA-08 | Fixed | Re-reading a brief that changes the organization, industry, offering or location lists the edits made for the old context, with Keep or Refresh per item and for all. Refresh can be undone. | Browser: London re-brief offers refresh; refreshed letter fits Brightpath. |
| QA-09 | Fixed | New health rules: empty context fields, empty or duplicate stage and people names, empty welcome letter, product brief, event title or text, all actions off, no leadership style action, a target far from the template's design and a zero deal value. Blanks and duplicates are errors and block publishing; the Review step lists them before creating. | Unit: all 10 probes flagged. |
| QA-10 | Fixed | Organization found at sentence start, after a capitalised For, in lower case, after @, and with trailing punctuation removed. Quoted words only count when they look like names. Technologies reads as IT and Industries as manufacturing. A Hindi brief now yields banking and Mumbai and a notice that Genie reads any language. | Unit. |
| QA-11 | Fixed | Negation around firing ("don't let them fire", "exclude the firing action") and tone ("not formal", "casual"). Two session lengths, both kinds of buyers, or both easier and harder become one-click clarifying questions; Genie can settle them. | Unit. |
| QA-12 | Fixed | Replacing an edited letter or product brief asks first. Genie regeneration of stages, events and backgrounds skips items the author edited and says so. Try another version, New names, Genie changes and Refresh all have Undo. | Browser: confirm, replace, undo restores the author's letter. |
| QA-13 | Fixed | While Genie reads, the brief is locked and the button row shows elapsed time and Stop. After 20 seconds the built-in rules take over with a message. | Browser: hung Genie, Stop falls back to rules. |
| QA-14 | Fixed | When Genie's version breaks field tokens, the author sees "Genie's version used a field we do not recognise, so we kept yours. Try again." and the failure is logged to the console for the AI team. | Browser. |
| QA-15 | Fixed | A synchronous in-flight guard covers brief reading and every regeneration. | Browser: double click makes one call. |
| QA-16 | Fixed | The store listens for other tabs' saves and merges per simulation (newest copy wins, deletions are respected); every write also merges with what is stored. | Unit: merge rules. Browser: two tabs keep both changes. |
| QA-17 | Fixed | Pending saves are flushed on pagehide and when the page is hidden. | Browser. |
| QA-18 | Fixed | On phones the step list and Studio sections become a compact Step N of 6 menu. Every explained control gets a tap-to-reveal info button on touch screens, and tips are nudged back inside the viewport. | Browser at 360 px: Publish tip at 13 to 273 px. |
| QA-19 | Fixed | Drawers and modals move focus to their heading, trap Tab, close on Escape and return focus to the trigger. | Browser. |
| QA-20 | Fixed | Country name pools for 32 more markets (Kenya, Nigeria, South Africa, Ghana, Egypt, Saudi Arabia, Brazil, Mexico, Canada, France, Spain, Italy, Netherlands, Sweden, Poland, Turkey, China, Korea, Indonesia, Philippines, Vietnam, Thailand, Malaysia, Pakistan, Bangladesh, Ireland, Argentina, Colombia, Chile, Portugal, Georgia, New Zealand), with gendered family names where they differ. Other countries keep the regional style with a Review names hint. | Unit. |
| QA-21 | Fixed | Items written for any industry show a Generic for your industry badge in the flow. For an industry without a pack, the Context step asks three optional questions (competitor, two other offerings, a typical setback) that fill the competitor, portfolio and crisis event. | Unit and browser. |
| QA-22 | Fixed | The brief is stored on the draft record for the author only. It is no longer in the definition, in published versions or in the exported file; older drafts are migrated. | Browser and unit. |
| QA-23 | Partly fixed | Settings shows a Your data notice (browser-only storage, what is sent to Genie and when, where the brief lives). Accounts, tenant isolation, audit log, retention and an organization-level AI switch are production requirements, listed in the product spec. | Browser: notice present. |
| QA-24 | Fixed | Defaults that are not asked show Suggested. Needs you appears only on questions that are asked and unanswered. | Browser. |
| QA-25 | Fixed | Unapplied organization profile changes are kept per simulation across tabs and sections, with a Not applied yet notice, Apply and Discard changes, and a banner on the other Story tabs. | Browser. |
| QA-26 | Fixed | The target message now says what it means: set at about three quarters of what a leader with perfect insight reaches, and what guessing reaches. A target from the brief is kept and checked, with the hit rate and a one-click fair alternative. | Browser: brief target 60 kept, checked, fair alternative offered. |
| QA-27 | Fixed | Industry-specific openings ("a young, fast-growing bank", "a challenger telecom operator"). Legacy phrasing such as "go-to guy" and "is a Finance Major" is rewritten when tailoring away from the legacy storyline. | Unit. |
| QA-28 | Fixed | A collapsible Your brief panel on every step, with Edit brief and, when the brief changed, Read it again. Set up without a brief skips straight to the questions. | Browser. |
| QA-29 | Fixed | Session undo and redo in the Studio (Undo button, Ctrl+Z or Cmd+Z outside text boxes, Ctrl+Shift+Z), with rapid typing merged into one step. | Browser: applying Singapore and undoing restores INR. |
| QA-30 | Fixed | Group play, leaderboard, LTI, SCORM and languages carry a Planned label and explain that the choice is saved for the production runtime. | Browser. |
| QA-31 | Partly fixed | The country search is an accessible combobox (no datalist), deep copies fall back when structuredClone is missing, and style pills fall back when color-mix is unsupported. Firefox and WebKit runs still need a device lab; supported browsers are listed in the product spec. | Browser (Chromium): combobox keyboard pick. |
| QA-32 | Fixed | Without Genie the flow says Create from a brief, shows a Genie offline status (Genie on when available) and adapts copy and tooltips. | Browser. |
